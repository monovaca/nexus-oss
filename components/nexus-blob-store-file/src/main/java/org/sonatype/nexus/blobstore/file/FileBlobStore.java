    ...
    implements BlobStore
{
  ...
  private BlobIdProvider blobIdProvider;
  ...
  public Blob create(final InputStream inputStream, final Map<String, String> headers) throws BlobStoreException {
    ...
    final BlobId blobId = blobIdProvider.createBlobId();
    ...
    try (final BlobLock contentLock = lockProvider.exclusiveLock(blobId)) {
      ...
      if (listener != null) {
        listener.blobCreated(blobId, blob.getHeaders(), blob.getMetrics(),
            "Blob " + blobId + " written to " + contentPath(blobId));
      }
  ...
  public Blob get(final BlobId blobId) throws BlobStoreException {
    ...
    try (final BlobLock lock = lockProvider.readLock(blobId)) {
      ...
      final FileBlob blob = new FileBlob(blobId, contentPath(blobId), headerPath(blobId));
      if (listener != null) {
        listener.blobAccessed(blobId, blob.getHeaders(), blob.getMetrics(), null);
      }
      return blob;
    }
  }
  ...
  public boolean delete(final BlobId blobId) throws BlobStoreException {
    ...
    try (final BlobLock lock = lockProvider.exclusiveLock(blobId)) {
      ...
      if (delBlob != delHeader) {
        logger.error("Deleting blob " + blobId + " : " + (!delBlob ? "content" : "header") + " file was missing.");
      }
  ...
  public void setBlobIdProvider(final BlobIdProvider blobIdProvider) {
    this.blobIdProvider = blobIdProvider;
  }
